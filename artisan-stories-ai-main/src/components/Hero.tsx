      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-4xl">
          ...
          <h1 className="text-6xl md:text-7xl font-bold mb-6 leading-tight">
            <span className="text-primary-foreground">{t('hero.connecting')}</span><br />
            <span className="bg-gradient-secondary bg-clip-text text-transparent">{t('hero.indianArtisans')}</span><br />
            <span className="text-primary-foreground">{t('hero.toTheWorld')}</span>
          </h1>
          
          <p className="text-xl md:text-2xl text-gray-800 dark:text-gray-200 mb-8 max-w-2xl leading-relaxed">
            {t('hero.description')}
          </p>
          ...
          <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 mb-12">
            ...
            >
              {t('button.browseCrafts')}
            </Button>
            <Button 
              size="lg" 
              variant="craft" 
              className="text-lg px-8 py-6"
              onClick={() => document.getElementById('login-dialog')?.click()}
            >
              {t('button.sellYourCraft')}
            </Button>
          </div>
