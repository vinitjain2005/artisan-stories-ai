        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => {
            ...
            return (
              <Card 
                key={index} 
                className="border-border hover:shadow-elegant transition-all duration-300 group"
                tabIndex={0}
                role="button"
                aria-label={`Feature: ${feature.title}`}
                onClick={() => {
                  const section = feature.title === "AI-Powered Descriptions" ? "about" :
                                 feature.title === "Hindi + English Support" ? "artisans" :
                                 feature.title === "Fair Price Suggestions" ? "marketplace" :
                                 feature.title === "Easy Product Upload" ? "artisans" :
                                 feature.title === "Social Media Ready" ? "marketplace" :
                                 "artisans";
              ...
              >
                <CardHeader className="text-center pb-4">
                  <div 
                    className={`inline-flex p-3 rounded-full bg-gradient-primary/10 mb-4 ${feature.color} group-hover:scale-110 transition-transform`}
                    aria-hidden="true"
                  >
